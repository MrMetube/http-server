...
main :: proc () {
    ...
    for {
        ...
        client, ok := accept(server.socket)
        if !ok do continue
...
respond_and_close :: proc (client: net.TCP_Socket, sb: ^strings.Builder, response: ^Response) {
...
accept :: proc (socket: net.TCP_Socket) -> (net.TCP_Socket, bool) {
    ok := true
    client, _, accept_error := net.accept_tcp(socket)
    if accept_error != nil {
        fmt.printf("Error: failed to accept a client tcp on '%v': %v\n", address_and_port_from_socket(socket), accept_error)
        net.close(client)
        ok = false
    }
