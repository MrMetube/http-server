...
main :: proc() {
    ...
    if handle_running_exe_gracefully(server_exe, .Kill) {
        build := true
        if (tasks & {.debugger }) != {} {
            if !did_change(server_exe_path, server_dir, `.\`) {
    ...
    if handle_running_exe_gracefully(client_exe, .Kill) {
        build := true
        if (tasks & {.debugger }) != {} {
            if !did_change(client_exe_path, client_dir, `.\`) {
                fmt.println("INFO: No changes detected. Skipping build.")
                build = false
            }
        }
    ...
    if .debugger in tasks {
        ...
        if .run in tasks {
            ...
            os.change_directory(`..\`+run_dir)
            append(&cmd, `..\`+build_dir+server_exe)
            run_command(&cmd, async = &procs, stdout = os2.stdout)
        }
    }
...
File_or_String :: union {^os2.File, ^string}

run_command :: proc (cmd: ^Cmd, or_exit := true, keep := false, stdout: File_or_String = nil, stderr: File_or_String = nil, async: ^Procs = nil) -> (success: bool) {
    fmt.printfln(`CMD: %v`, strings.join(cmd[:], ` `))
    
    process_description := os2.Process_Desc { command = cmd[:] }
    if file, file_ok := stdout.(^os2.File); file_ok do process_description.stdout = file
    if file, file_ok := stderr.(^os2.File); file_ok do process_description.stderr = file
    process: os2.Process
    ...
    if async == nil {
        if output != nil {
            if inner, ok := stdout.(^string); ok do inner ^= cast(string) output
            else do fmt.println(cast(string) output)
        }
        
        if error != nil {
            if inner, ok := stderr.(^string); ok do inner ^= cast(string) error
